# UI/UX Requirements: AI Fairness Dashboard

Since the interactive visual design environment is currently unavailable, this document provides the **Logical Wireframe Architecture** and **Interaction Model** for the AI Fairness & Bias Detection Toolkit.

## 1. Persona-Based Navigation
- **Workspace Selector**: Choose between different projects (e.g., "Credit Scoring Model," "Hiring Algorithm").
- **Persona Toggle**:
    - **Data Scientist**: Default view. Detailed metrics, SHAP values, and API logs.
    - **Compliance/Legal**: Simplified "Traffic Light" view (Green/Yellow/Red) for regulatory reporting.

---

## 2. Page: Executive Overview (The "At-a-Glance" View)
### 2.1 Global Fairness Score
- **Widget**: Circular progress bar showing an aggregate "Fairness Health" percentage.
- **Logic**: Weighted average of Disparate Impact, Statistical Parity, and Equalized Odds across all protected groups.

### 2.2 Bias Hotspots (Heatmap)
- **X-Axis**: Protected Attributes (Race, Gender, Age, Disability Status).
- **Y-Axis**: Outcome Categories (Loan Approved, High Premium, Resume Shortlisted).
- **Visual**: Cells colored by risk level. Clicking a cell drill-down into specific data points.

---

## 3. Page: Deep-Dive Analysis (Interactive Auditing)
### 3.1 Disparate Impact Chart
- **Type**: Grouped Bar Chart.
- **Controls**: Dropdown to select "Privileged Group" (Reference Group).
- **Visual Alert**: A horizontal 80% line (standard regulatory benchmark). Bars falling below this line are automatically flagged with a red border.

### 3.2 Feature Importance & Proxy Checker
- **Widget**: Horizontal bar chart showing SHAP values.
- **Logic**: Highlights variables that have high correlation with protected attributes (e.g., "Zip Code" being a proxy for "Race").
- **Action**: "Suggest Drop" button to temporarily exclude a proxy variable and see the recalculated fairness impact.

---

## 3.3 The Remediation Sandbox (What-If Analysis)
- **Top Panel**: Current Model Performance (Accuracy, Precision, Recall).
- **Side Panel**: Remediation Sliders.
    - **Resampling Slider**: Adjust the weight of minority class samples.
    - **Threshold Slider**: Manually adjust the decision threshold (e.g., 0.5 to 0.45) for specific demographic groups to equalize the False Positive Rate.
- **Live Preview**: As sliders move, the Accuracy vs. Fairness trade-off chart updates in real-time.

---

## 4. Flagging & Reporting Mechanism
- **The "Audit Trail"**: A chronological feed of flags raised by the system or human auditors.
- **Remediation Status**: "Open," "Assigned," "Resolved (Model Retrained)," "Risk Accepted (With Justification)."
- **Export Module**: One-click generation of a "Model Card" (PDF) containing all fairness metrics, training data distribution, and intended use cases for compliance filing.

---

## 5. IDE Extension Logic (Add-on)
- **Notification**: Subtle toast alert in the IDE footer: *"High disparate impact (0.64) detected in 'train.py'. Click to view remediation playbook."*
- **Inline Linting**: Squiggly lines under code blocks that define sensitive features without proper anonymization decorators.
